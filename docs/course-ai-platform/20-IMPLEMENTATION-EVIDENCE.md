...
| Circle installation handoff | B / partial | `/install/circle` provides a tenant-safe operator guide and a paste-ready snippet, without trusting `window.circleUser` as authorization. A mobile/app link fallback to the hosted assistant (`/c/{slug}`) is explicit. Installation in Estie's real Circle community, server-verified Circle identity, CSP compatibility and workflow/webhook acceptance remain pending. **Superseded evidence, corrected 2026-07-31:** this row originally cited `public/integrations/circle-learningbot.js`, a "static self-contained launcher script that opens the authenticated LearningBot workspace". That artifact and its bundled `public/integrations/widget-runtime.js` copy were deleted on 2026-07-31. They had been orphaned since Phase 7 landed: nothing executable referenced them, and they instructed a `data-widget-key` attribute that the shipped runtime does not read (it auto-mounts from `data-tenant`, `packages/widget-runtime/src/index.ts:2062`), so anyone following the docstring got a silent no-op while the files stayed publicly served. The claim above is re-evidenced against the delivery path that actually ships. | `/install/circle` (`apps/console/src/app/install/circle/page.tsx:83-86`, emits `script.src = "…/widget.js"` + `script.dataset.tenant`); `apps/console/src/app/widget.js/route.ts`; `apps/console/src/app/widget.js/embed-prelude.ts`; `apps/console/test/circle-install.test.ts`; production build |
