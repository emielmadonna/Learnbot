...
| VOI-01, VOI-02 | B / partial | The authenticated `/app/conversation` supports a real bounded push-to-talk turn: a selected-tenant/membership preflight runs before microphone activation; negotiated WebM/MP4/M4A/AAC audio is transcribed by `gpt-4o-mini-transcribe`; the transcript follows the same durable grounded response path as text; and the tenant-authorized saved answer is spoken by `gpt-4o-mini-tts` with the `marin` voice. The calm presentation exposes recording/transcribing/thinking/speaking state, locks context throughout an active turn, invalidates stale work, supports interruption/manual playback recovery/text fallback, releases resources, and discloses synthetic voice and push-to-talk limitations. Requests have pre-parse size/type policy and an honest bounded process-instance safety quota. This is not yet continuous WebRTC Realtime voice, durable distributed metering or measured low-latency barge-in. | `pnpm --filter @course-ai/console test`, 45 cases; voice contract/policy/quota tests; production build |
...
| Durable schema | A/C / partial | Twenty-six ordered migrations define 42 RLS-enabled public tables plus private operation/claim state for execution, identity, upload, onboarding, durable learning, owner claims, grounded retrieval, conversations, hybrid vectors and quarantine uploads. Structural verification passes and the hosted ledger includes 0001–0026. Existing eleven hosted transactional suites pass with rolled-back fixtures; the new hybrid/upload migrations have hosted structural, privilege and live-corpus evidence but not a new full transactional suite claim. The live security advisor reports only intentional signed-in `SECURITY DEFINER` warnings; performance indexing and policy-consolidation advisories remain operational backlog. Backup/PITR restore, load and failover evidence remain pending. | `pnpm supabase:verify`; hosted migration/advisor/privilege inspection; hosted SQL acceptance transcript |
...
- The optimized local Next.js build generated **56 application entries**,
...
- `verify:dev` continues to cover the development API, authoring, Creator
  ...
  `pnpm --filter @course-ai/console smoke:intelligence` and
  `pnpm --filter @course-ai/console smoke:privacy`.
- `pnpm supabase:verify` passes for **26 ordered migrations and 42 public
  tables**. The hosted migration ledger includes 0001–0026 and all **eleven
...
- The console test suite passes **45/45** and the MCP package passes **18/18**.
