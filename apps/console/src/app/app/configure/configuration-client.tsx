...
function ConfigurationForm({ initial }: { initial: TenantConfiguration }) {
  ...
  const dirty = JSON.stringify(draft) !== JSON.stringify(draftFrom(configuration)) || credentialDraft.length > 0 || clearCredential;
    ...
    try {
      const response = await fetch("/api/app/configure", {
        ...
        body: JSON.stringify({
          ...
          ...draft,
          ...(credentialDraft ? { apiKey: credentialDraft } : {}),
          ...(clearCredential ? { clearApiKey: true } : {}),
        }),
      });
  ...
  return (
    <section className={styles.section} id="configuration" aria-labelledby="configuration-heading">
      ...
      <article className={styles.card}><h3>Provider and model</h3><div className={styles.fields}><label>Provider<select value={draft.provider.provider} onChange={(event) => update("provider", { ...draft.provider, provider: event.target.value as TenantConfiguration["provider"]["provider"] })}>{PROVIDERS.map((provider) => <option key={provider.id} value={provider.id}>{provider.label}</option>)}</select></label><label>Model<input value={draft.provider.model} onChange={(event) => update("provider", { ...draft.provider, model: event.target.value })} /></label></div><div className={styles.boundary}><strong>{configuration.credentials.configured ? "Tenant provider access: available" : "Deployment provider fallback: active"}</strong><span>{configuration.credentials.vaultReferencePresent ? "This workspace uses its own encrypted Vault credential." : "No tenant credential is registered; the deployment provider is used."}</span><small>Paste a new key to replace the tenant credential. The key is sent once to the server-side Vault boundary and is never returned or persisted in this browser.</small><label>Provider API key<input type="password" autoComplete="new-password" value={credentialDraft} placeholder={configuration.credentials.configured ? "Enter a replacement key" : "Paste provider key"} onChange={(event) => { setCredentialDraft(event.target.value); setClearCredential(false); setStatus("Unsaved credential change"); }} /></label><button type="button" onClick={() => { setCredentialDraft(""); setClearCredential(true); setStatus("Tenant credential will be revoked on save"); }} disabled={!configuration.credentials.configured && !credentialDraft}>Remove tenant credential</button></div></article>
