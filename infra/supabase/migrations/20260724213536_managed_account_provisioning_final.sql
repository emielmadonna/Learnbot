-- Final managed-account provisioning boundary.
-- This keeps the Auth credential outside application storage, repairs the
-- account-list query, and preserves staged/idempotent RPC diagnostics.
...
begin
  ...
  -- Serialize retries and concurrent requests for the same Auth user.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(target_auth_user_id::text, 20260724)
  );

  stage := 'caller_context';
  ...
  -- A lost RPC response can be retried with the same temporary password.
  -- Treat a complete existing managed account as a successful idempotent retry.
  stage := 'idempotency_check';
  if exists (
    select 1
    from app_private.user_access_accounts a
    join app_private.supabase_auth_principal_links l
      on l.auth_user_id = a.auth_user_id
     and l.principal_id = a.principal_id
    join public.identity_memberships m
      on m.tenant_id = a.tenant_id
     and m.membership_id = a.membership_id
     and m.principal_id = a.principal_id
    where a.auth_user_id = target_auth_user_id
      ...
      and a.email_normalized = normalized_email
      and m.role = target_identity_role
      and m.status = 'active'
