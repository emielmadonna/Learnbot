...
const modelPattern = /^[a-z0-9._:-]{2,120}$/iu;
...
Deno.serve(async (request: Request) => {
  ...
  if (credentialResult.ok !== true) {
    const code = credentialResult.code === "tenant_credential_not_configured"
      ? "tenant_credential_not_configured"
      : credentialResult.code === "access_denied"
        ? "access_denied"
        : "credential_boundary_unavailable";
    return json({ ok: false, code });
  }
  const credential = typeof credentialResult.credential === "string"
